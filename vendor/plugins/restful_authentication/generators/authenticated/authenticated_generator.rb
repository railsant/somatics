  ...
  def manifest
    recorded_session = record do |m|
      ...
      m.template 'authenticated_system.rb',
                  File.join('lib', "#{file_name}_authenticated_system.rb")

      m.template 'authenticated_test_helper.rb',
                  File.join('lib', "#{file_name}_authenticated_test_helper.rb")
      ...
      unless options[:skip_routes]
        ...
        m.route_name("#{file_name}_signup",   "/#{model_controller_controller_name}/signup",   {:controller => model_controller_plural_name, :action => 'new'})
        m.route_name("#{file_name}_register", "/#{model_controller_controller_name}/register", {:controller => model_controller_plural_name, :action => 'create'})
        m.route_name("#{file_name}_login",    "/#{model_controller_controller_name}/login",    {:controller => controller_controller_name, :action => 'new'})
        m.route_name("#{file_name}_logout",   "/#{model_controller_controller_name}/logout",   {:controller => controller_controller_name, :action => 'destroy'})
      end
    end
    ...
    when "generate"
      ...
      puts %(    map.signup '#{model_controller_controller_name}/signup', :controller => '#{model_controller_file_name}', :action => 'new')
      puts %(    map.login  '#{model_controller_controller_name}/login',  :controller => '#{controller_file_name}', :action => 'new')
      puts %(    map.logout '#{model_controller_controller_name}/logout', :controller => '#{controller_file_name}', :action => 'destroy')
      if options[:include_activation]
